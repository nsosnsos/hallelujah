...
def article(article_url):
    ...
    if not article.is_public and (
        not current_user.is_authenticated or article.user_id != current_user.id
    ):
        return redirect_back("main.index")
    return render_template(
        "main/view_article.html",
        article=article,
        is_self=(current_user == article.author),
    )
...
def articles():
    return render_template(
        "main/articles.html", user=current_user, is_self=True
    )
...
def delete_article(article_url):
    ...
    if (
        ...
        or not Article.delete_article(article_id=article.id)
    ):
        flash("Failed to find the article!")
    else:
...
def manage_medias(current_path):
    ...
    files = (
        Media.query.filter(Media.path == current_path)
    ...
    if form.validate_on_submit():
        ...
        flash(
            "Directory "
            + os.path.join(current_path, target_dir)
...
def get_file(filename):
    ...
    if not media or (
        ...
        and (
            ...
            or current_user.name != media.author.name
        )
    ):
        return Response("", status=204, mimetype="text/xml")
    full_path_name = os.path.join(
        _get_original_path(), media.path, media.filename
    )
...
def get_thumbnail(filename):
    ...
    full_path_name = os.path.join(
        _get_thumbnail_path(), media.path, media_filename
    )
    if not os.path.isfile(full_path_name):
        return Response("", status=204, mimetype="text/xml")
    return send_file(
        full_path_name, as_attachment=False, download_name=filename
    )
...
def _delete_file(media):
    ...
    full_path_name = os.path.join(
        _get_thumbnail_path(), media.path, thumbnail_filename
    )
    if os.path.isfile(full_path_name):
        os.remove(full_path_name)
        current_app.logger.info(f"deleted file {full_path_name}")
...
def delete_media_directory(current_path):
    ...
    for media in medias:
        if (
            not media
            or media.author.name != current_user.name
            or not Media.delete_media(media.uuidname)
        ):
...
@login_required
def resources():
    user_resources = Resource.query.filter(
        Resource.user_id == current_user.id
    ).order_by(Resource.rank.desc(), Resource.id.asc())
    json_resources = [resource.to_json() for resource in user_resources]
    categories = sorted(
        list(set([resource["category"] for resource in json_resources]))
    )
    ...
    return render_template(
        "main/resources.html", resources=resources, local_icon=local_icon
    )
...
def delete_resource(resource_id):
    ...
    if (
        not resource
        or resource.user_id != current_user.id
        or not Resource.delete_resource(resource_id=resource_id)
    ):
        flash(
            "Failed to delete the resource:[{}][{}]!".format(
                resource.title, resource.uri
            )
        )
    else:
        flash(
            "Resource " + resource.title + " " + resource.uri + " is deleted!"
        )
    return redirect_back()
    ...
    try:
        ...
        if "text/html" in content_type or "text/javascript" in content_type:
            ...
            for tag in soup.find_all(["a", "link", "form"], href=True):
                tag["href"] = (
                    f'/proxy?url={url_percent_encoding(url, tag["href"])}'
                )
            for form in soup.find_all(["form"], action=True):
                form["action"] = (
                    f'/proxy?url={url_percent_encoding(url, form["action"])}'
                )
            for tag in soup.find_all(["img", "script", "link"], src=True):
                tag["src"] = (
                    f'/proxy?url={url_percent_encoding(url, tag["src"])}'
                )
            return Response(
                str(soup), headers=headers, content_type=content_type
            )
        else:
            return Response(
                rsp.content, headers=headers, content_type=content_type
            )
